...
namespace Peg.Util
{
    ...
    /// if part of a prefab. Also note that custom inspectors don't support generics so you must specify a concrete,
    /// non-generic dictionary and a concrete dictionary drawer in order to see these values in the inspector.
    /// 
    /// BUG ALERT: There is a HUGE bug in the 'Keys' and 'Values' properties. They return the first value in their associated bucket
    ///             without regards to the actual number of pre-allocated, but unused, elements in there.
    /// 
    /// </summary>
    ...
    public class HashMap<TKey, TValue> : IDictionary<TKey, TValue>, IDictionary
    {
        ...
        #region IDictionary<>
        public delegate void ValueChangedEvent(TKey key);
        ...
        {
            Insert(key, default(TValue), false);
        }
        ...
        private void Insert(TKey key, TValue value, bool add)
        {
            ...
            for (int i = _Buckets[index]; i >= 0; i = _Next[i])
            {
                if (_HashCodes[i] == hash && _Comparer.Equals(_Keys[i], key))
                {
                    ...
                    _Values[i] = value;
                    _Version++;
                    if (OnValueChanged != null) OnValueChanged(key);
                    return;
                }
                num1++;
            }
        ...
        /// <summary>
        /// This was added for more efficient looping through entries without
        /// having to use iterators (which produce garbage in Unity).
        /// 
        /// DO NOT MODIFY THIS LIST OR CHAOS WILL ENSUE!
        /// 
        /// Also note that the size of the array might be bigger than the number of entries in the dictionary.
        /// DON'T USE IT FOR DETERMINIG LENGTH OF DICTIONARY!
        /// 
        /// </summary>
        public TKey[] SimpleKeys
        {
            get { return _Keys; }
        }
        ...
        /// 
        /// </summary>
        public TValue[] SimpleValues
        {
            get { return _Values; }
        }

        /// <summary>
        /// Added for more efficient looping through entries without
        /// having to use iterators (which produce garbage in Unity).
        /// Unlike <see cref="SimpleValues"/> this list will not return
        /// dummy allocations and instead will provide the exact list of
        /// elements contained in this HashMap's keys array. The list
        /// should not be modified in order to avoid future allocations.
        /// 
        /// You should consider this list volitile and not thread safe.
        /// Do not cache this list.
        /// </summary>
        public List<TKey> KeysNonAlloc
        {
            get
            {
                var list = SharedArrayFactory.RequestTempList<TKey>();
        ...
        /// Unlike <see cref="SimpleValues"/> this list will not return
        /// dummy allocations and instead will provide the exact list of
        /// elements contained in this HashMap's values array. The list
        /// should not be modified in order to avoid future allocations.
        /// 
        /// You should consider this list volitile and not thread safe.
        /// Do not cache this list.
        /// </summary>
        public List<TValue> ValuesNonAlloc
        {
            get
            {
                var list = SharedArrayFactory.RequestTempList<TValue>();
                for(int i = 0; i < Count; i++)
                    list.Add(_Values[i]);
                return list;
            }
        ...
        {
            private readonly HashMap<TKey, TValue> _Dictionary;
            ...
            internal Enumerator(HashMap<TKey, TValue> dictionary)
            {
